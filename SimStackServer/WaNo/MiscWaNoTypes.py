...
def WaNoListEntry_from_folder_or_zip(
    file_or_dir_name: str, ignore_icon=True
) -> WaNoListEntry:
    ...
    if file_or_dir_name_path.is_dir():
        ...
        raise NotADirectoryError(
            f"Directory {file_or_dir_name_path} was neither a directory or a file ending with .zip."
        )
    icon = None
    if not ignore_icon:
        raise NotImplementedError(
            "Currently WaNoListEntry_from_folder_or_zip does not parse icon. Please implement."
        )
