...
class ClusterSettingsProvider:
    ...
    def _get_settings_folder(self) -> Path:
        ...
        ucd = Path(
            appdirs.user_config_dir(
                appname="SimStack", appauthor="Nanomatch", roaming=False
            )
        )
    ...
    def _parse_settings(self):
        self._settings_container = get_cluster_settings_from_folder(
            self._get_settings_folder()
        )

    def write_settings(self):
        save_cluster_settings_to_folder(
            self._get_settings_folder(), self._settings_container
        )
