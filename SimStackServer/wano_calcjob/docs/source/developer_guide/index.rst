...
``wano_calcjob`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``wano_calcjob`` repository (preferably using ``wano_calcjob`` as the project name)

The documentation is now available at `wano_calcjob.readthedocs.io <http://wano_calcjob.readthedocs.io/>`_.
...
.. note:: 
   ...
   When updating the plugin package to a new version, remember to update the version number both in ``setup.json`` and ``wano_calcjob/__init__.py``.
