...
from SimStackServer.wano_calcjob.wano_calcjob.wano_calcjob_exceptions import (
    UnknownTypeError,
)
...
class WaNoCalcJob(CalcJob):
    ...
    def _guess_type(input_value):
        ...
        if isinstance(input_value, bool):
            ...
            raise UnknownTypeError(
                "Could not find type of %s: %s" % (input_value, type(input_value))
            )
    ...
    def _build_spec_from_wano(cls, wfxml, spec):
        ...
        for myfile in output_files:
            spec.output(
                cls.clean_path(cls.dot_to_none(myfile)), valid_type=SinglefileData
            )

        exec_command = wmr.exec_command
        spec.input(
            "metadata.options.exec_command", valid_type=str, default=exec_command
        )
    ...
    def _set_caches(cls):
        ...
        cls._cached_inputs = wano_namespaces_and_vars_dict[
            "input_types_by_cleaned_path"
        ]
        cls._cached_outputs = wano_namespaces_and_vars_dict[
            "output_types_by_cleaned_path"
        ]
        cls._cached_output_namespaces = wano_namespaces_and_vars_dict[
            "output_namespaces"
        ]
        ...
        cls._cached_extra_inputfiles = wano_namespaces_and_vars_dict[
            "extra_inputfile_paths"
        ]
        cls._cached_simstack_to_aiida_pathmap = wano_namespaces_and_vars_dict[
            "simstack_to_aiida_pathmap"
        ]
        cls._cached_aiida_to_simstack_pathmap = wano_namespaces_and_vars_dict[
    ...
    def prepare_for_submission(self, folder):
        ...
        for localfile_path_without in self.extra_inputfiles():
            try:
                localfile_path = "static_extra_files." + self.dot_to_none(
                    localfile_path_without
                )
        ...
        local_copy_list.append(
            (
                self.inputs.static_extra_files.rendered_wanoyml.uuid,
                "rendered_wano.yml",
                "rendered_wano.yml",
            )
        )
