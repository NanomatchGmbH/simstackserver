...
class ExcitonPreProcessorCalcJob(WaNoCalcJob):
    _myxml = join(
        WaNoCalcJob.wano_repo_path(), "ExcitonPreProcessor", "ExcitonPreProcessor.xml"
    )
...
class DihedralParametrizer3_expertCalcJob(WaNoCalcJob):
    _myxml = join(
        WaNoCalcJob.wano_repo_path(),
        "DihedralParametrizer3_expert",
        "DihedralParametrizer3_expert.xml",
    )
...
class StokesShiftAnalysisCalcJob(WaNoCalcJob):
    _myxml = join(
        WaNoCalcJob.wano_repo_path(), "StokesShiftAnalysis", "StokesShiftAnalysis.xml"
    )
...
class lightforge2_analysisCalcJob(WaNoCalcJob):
    _myxml = join(
        WaNoCalcJob.wano_repo_path(), "lightforge2_analysis", "lightforge2_analysis.xml"
    )
...
class OrientationAnalysisCalcJob(WaNoCalcJob):
    _myxml = join(
        WaNoCalcJob.wano_repo_path(), "OrientationAnalysis", "OrientationAnalysis.xml"
    )
...
class DihedralParametrizer3CalcJob(WaNoCalcJob):
    _myxml = join(
        WaNoCalcJob.wano_repo_path(),
        "DihedralParametrizer3",
        "DihedralParametrizer3.xml",
    )
...
class VariableExporterCalcJob(WaNoCalcJob):
    _myxml = join(
        WaNoCalcJob.wano_repo_path(), "VariableExporter", "VariableExporter.xml"
    )
