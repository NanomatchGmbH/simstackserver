...
class XMLYMLInstantiationBase(object):
    ...
    def set_field_value(self, fieldname, value):
        ...
        assert isinstance(value, self._field_types[fieldname]), (
            "Value for field %s does not have the correct type" % fieldname
        )
    ...
    def dump_xml_to_file(self, filename):
        ...
        with open(filename, "wt") as infile:
            infile.write(
                etree.tostring(me, encoding="utf8", pretty_print=True).decode() + "\n"
            )
    ...
    if name == "WorkflowExecModule":
        ...
        if name.startswith("np."):
            ...
            assert np.issubdtype(
                classobj, np.number
            ), "Only allowed to import actual numbertypes"
            return classobj
        ...
        for myid, my_str in enumerate(self._storage):
            if isinstance(my_str, str):
                ...
                for key, item in vardict.items():
                    ...
                    self._logger.info(
                        "Replacing %s with %s and %s, Outcome was: %s"
                        % (my_str, key, item, current_string)
                    )
...
class Resources(XMLYMLInstantiationBase):
    ...
    _fields = [
        (
            "resource_name",
            str,
            _connected_server_text,
            "Name of the resource. Valid resource if unset",
            "a",
        ),
        ...
        (
            ...
            str,
            "",
            "Comma separated list specifying additional parameters not covered here.",
            "m",
        ),
        ...
        (
            ...
            str,
            "simstack_workspace",
            "Basepath where to execute workflows relative to home/username",
            "m",
        ),
        ...
        (
            "sw_dir_on_resource",
            str,
            "/home/nanomatch/nanomatch",
            "Software directory on cluster",
            "m",
        ),
        (
            "extra_config",
            ...
            "Filepath on cluster to configuration file required before Serverstart is possible",
            "m",
        ),
        ...
        (
            ...
            str,
            "",
            "SGE Parallel Environment. Only applicable to SGE queue",
            "m",
        ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert len(self.render_order()) == len(
            self._fields
        ), "Please add all fields to render_order."
    ...
    def overwrite_unset_fields_from_default_resources(
        ...
        queue = self.queue
        if self.resource_name == self._connected_server_text:
            self.set_field_value("basepath", default_resources.basepath)
...
class WorkflowExecModule(XMLYMLInstantiationBase):
    _fields = [
        ...
        (
            ...
            str,
            "unset",
            "Path to the output directory of this wfem in the workflow.",
            "a",
        ),
        (
            ...
            str,
            "",
            "Path to the output directory of the wfem where the results were originally computed.",
            "a",
        ),
        ...
        (
            ...
            str,
            None,
            "Command to be executed as part of BSS. Example: 'date'",
            "m",
        ),
        ...
        (
            ...
            str,
            "unstarted",
            "The directory this wfem was started in",
            "m",
        ),
        (
            "jobid",
            str,
            "unstarted",
            "The id of the job this wfem was started with.",
            "m",
        ),
        (
            ...
            str,
            "",
            "If this is a non-local job, the directory on the external directory is stored here.",
            "m",
        ),
    ]
    ...
    def fill_in_variables(self, vardict):
        ...
        for key, item in vardict.items():
            self._field_values["outputpath"] = self._field_values["outputpath"].replace(
                key, item
            )
    ...
    def rename(self, renamedict):
        myuid = self.uid
        if myuid not in renamedict:
            raise KeyError(
                "%s not found in renamedict. Dict contained: %s"
                % (myuid, ",".join(renamedict.keys()))
            )
    ...
    def _get_mamba_variables(self) -> Dict[str, str]:
        ...
        for com in ["mamba", "conda"]:
            try:
                ...
                proc = subprocess.run(
                    conda_command, capture_output=True, encoding="Utf-8"
                )
                if proc.returncode != 0:
                    self._logger.error(
                        f"Could not parse Conda variables. Stdout was: {proc.stdout}, Stderr: {proc.stderr}"
                    )
                    continue
                text = proc.stdout
    ...
    def _get_prolog_unicore_compatibility(self, resources):
        ...
        varstring = [
            f"export {var}={content}" for var, content in exported_variables.items()
        ]
        varstring = "\n".join(varstring)
    ...
    def run_jobfile(self, external_cluster_manager=None):
        ...
        if external_cluster_manager is not None:
            ...
            with external_cluster_manager.connection_context():
                ...
                ext_dir_abs = external_cluster_manager.put_directory(
                    self.runtime_directory,
                    ext_dir_name,
                    basepath_override=actual_resources.basepath,
                )
        ...
        try:
            ...
            if (
                actual_resources.custom_requests.strip() != ""
                and actual_resources.custom_requests != "None"
            ):
                ...
                for field in crsplit:
                    ...
                    if len(splitfield) < 2:
                        raise JobSubmitException(
                            "Could not interpret additional resource specifiers. They were: %s"
                            % c_requests
                        )
                    key = splitfield[0]
...
""" % (self._get_prolog_unicore_compatibility(actual_resources), self.exec_command)
            ...
            try:
                jobscript = clusterjob.JobScript(
                    ...
                    time=mytimestring,
                    nodes=int(actual_resources.nodes),
                    ppn=int(actual_resources.cpus_per_node),
                    mem=actual_resources.memory,
                    stdout=self.given_name + ".stdout",
                    stderr=self.given_name + ".stderr",
                ...
                if not do_internal and not do_aiida:
                    ...
                    if asyncresult.status == FAILED:
                        ...
                        raise JobSubmitException(
                            "Job failed immediately after submission."
                        )
                    ...
                    if actual_resources.queue not in ["default", "None", None]:
                        inputs["metadata"]["options"][
                            "queue_name"
                        ] = actual_resources.queue

                    if actual_resources.queue != "default":
                        inputs["metadata"]["options"]["max_wallclock_seconds"] = int(
                            actual_resources.walltime
                        )
                    ...
                    envdict["UC_PROCESSORS_PER_NODE"] = str(
                        actual_resources.cpus_per_node
                    )
                    envdict["UC_TOTAL_PROCESSORS"] = str(
                        actual_resources.cpus_per_node * actual_resources.nodes
                    )
                    ...
                    with open(runscript, "wt") as outfile:
                        outfile.write(
                            str(jobscript)
                            .replace("$SLURM_SUBMIT_DIR", self.runtime_directory)
                            .replace(
                                "#LOCAL_EXEC_HOSTFILE_PLACEHOLDER",
                ...
                server_submit_stderr = join(
                    self.runtime_directory, "submission_failed.stderr"
                )
                self._logger.error(
                    "Exception: %s. Writing traceback to: %s"
                    % (e, server_submit_stderr)
                )
                with open(server_submit_stderr, "wt") as outfile:
                    traceback.print_exc(file=outfile)
                    outfile.write("\n\n")
                    outfile.write(
                        "During this exception, the following events were logged:\n"
                    )
    ...
    def _recreate_asyncresult_from_jobid(self, jobid):
        actual_resources = self.resources
        assert (
            not actual_resources.queueing_system == "Internal"
    ...
    def completed_or_aborted(self):
        ...
        if actual_resources.queueing_system not in ["Internal", "AiiDA"]:
            try:
                asyncresult = self._recreate_asyncresult_from_jobid(self.jobid)
    ...
    def get_output_variables(self, render_report=False):
        ...
        if render_report:
            ...
            try:
                ...
                report_renderer = ReportRenderer.render_everything(
                    self.runtime_directory, render_report
                )
            except Exception:
        ...
        if report_renderer is None:
            ...
            try:
                report_renderer = ReportRenderer.render_everything(
                    self.runtime_directory, False
                )
            except Exception:
                self._logger.exception(
                    "Could not create report_renderer for variable instruction. Something went very wrong."
                )
                raise WorkflowAbort(
                    "Could not create report_renderer for variable instruction. Something went very wrong."
                )
    ...
    def get_rendered_body_html(self):
        ...
        if self._rendered_body_html is None or self._rendered_body_html == "":
            ...
            body_html_parsed = lxml.html.fragment_fromstring(
                self._rendered_body_html, create_parent="p"
            )
...
class DirectedGraph(object):
    ...
    def get_running_jobs(self):
        outnodes = [
            node
            for node in self._graph
            if self._graph.nodes[node]["status"] == "running"
        ]
        return outnodes
    ...
    def get_success_jobs(self):
        outnodes = [
            node
            for node in self._graph
            if self._graph.nodes[node]["status"] == "success"
        ]
        return outnodes
    ...
    def get_failed_jobs(self):
        outnodes = [
            node
            for node in self._graph
            if self._graph.nodes[node]["status"] == "failed"
        ]
        return outnodes
    ...
    def get_next_ready(self):
        ...
        for a in nx.dfs_tree(self._graph):
            ...
            if nodes[a]["status"] == "unstarted":
                ...
                for pred in predecessors:
                    foundpred = True
                    self._logger.debug(
                        "pred %s status: %s" % (pred, nodes[pred]["status"])
                    )
        ...
        outnodes = [
            node for node in self._graph if self._graph.nodes[node]["status"] == "ready"
        ]
        return outnodes
...
class SubGraph(XMLYMLInstantiationBase):
    _fields = [
        (
            ...
            WorkflowElementList,
            None,
            "List of Linear Workflow Elements (this can also be fors or splits)",
            "m",
        ),
        (
            "graph",
            ...
            "another workflow this workflow does not need to be encoded here.",
            "m",
        ),
    ]
    ...
    def rename_all_nodes(self, explicit_overrides=None) -> dict:
        ...
        for val in rename_dict.keys():
            assert (
                val not in self.graph._graph.nodes
            ), "Found reference to old node in graph, which should not be there."
        ...
        for element in self.elements:
            if hasattr(element, "rename"):
                ...
                self._logger.warning(
                    "Element %s does not have explicit rename function yet. Please add even if empty."
                    % type(element)
                )
        return rename_dict
...
class IfGraph(XMLYMLInstantiationBase):
    _fields = [
        (
            ...
            SubGraph,
            None,
            "Graph to instantiate If in case condition is true",
            "m",
        ),
        (
            "falsegraph",
            SubGraph,
            None,
            "Graph to instantiate If in case condition is false",
            "m",
        ),
        (
            "true_final_ids",
            StringList,
            [],
            "These are the final uids of the subgraph. Required for linking copies of the subgraph.",
            "m",
        ),
        (
            "false_final_ids",
        ...
        (
            ...
            str,
            "",
            "UID, which will be completed, once IfGraph is completed. Every subgraph will link to this node",
            "a",
        ),
    ...
    def fill_in_variables(self, vardict):
        ...
        for key, value in vardict.items():
            if key.startswith("${"):
                self._field_values["condition"] = self._field_values[
                    "condition"
                ].replace(key, value)
    ...
    def _connect_subgraph(self, condition_is_true_or_false):
        ...
        new_connections = []
        if "temporary_connector" not in rename_dict:
            raise WorkflowAbort(
                "mygraph did not contain start id temporary_connector. Renamed keys were: %s"
                % (",".join(rename_dict.keys()))
            )
        for uid in final_ids:
            if uid not in rename_dict:
                raise WorkflowAbort(
                    "mygraph did not contain final id %s. Renamed keys were: %s"
                    % (uid, ",".join(rename_dict.keys()))
                )
        for uid in final_ids:
...
class ForEachGraph(XMLYMLInstantiationBase):
    _fields = [
        ...
        (
            ...
            StringList,
            [],
            "Files and globpatterns to iterate over",
            "m",
        ),
        (
            "iterator_variables",
            StringList,
            [],
            "Variables, which have to be iterated over",
            "m",
        ),
        (
            "iterator_definestring",
            str,
            "",
            "String, which defines variable iterators",
            "m",
        ),
        ...
        (
            ...
            str,
            "",
            "UID, which will be completed, once ForEachGraph is completed. Every subgraph will link to this node",
            "a",
        ),
        ("uid", str, "", "UID of this Foreach", "a"),
    ]
    ...
    def fill_in_variables(self, vardict):
        ...
        for key, value in vardict.items():
            self._field_values["iterator_definestring"] = self._field_values[
                "iterator_definestring"
            ].replace(str(key), str(value))
    ...
    def _resolve_file_iterator(self, base_storage):
        ...
        for myfile_rel in relfiles:
            ...
            if isglob:
                ...
                allfiles += [myfile]
            self._logger.info(
                "Iterating over %s when looking for %s."
                % (", ".join([str(e) for e in allfiles]), myfile)
            )
    ...
    def _multiply_connect_subgraph(self, iterator_names, resolved_files):
        ...
        for iterator_value, myvalues in enumerate(resolved_files):
            ...
            if len(myvalues) != len(iterator_names):
                out = StringIO()
                out.write(
                    "Mismatch between iterator values and number of declared iterators."
                )
...
class WhileGraph(XMLYMLInstantiationBase):
    ...
    def resolve_connect(self, base_storage, input_variables, output_variables):
        ...
        for vardict in replacedict, output_variables, input_variables:
            for key, item in vardict.items():
                ...
                condition = condition.replace(key, str(item))
        self._logger.info(
            "While Condition %s resolved to %s" % (self.condition, condition)
        )
        # outcome = literal_eval(condition)
        try:
            outcome = eval(condition)
    ...
    def _multiply_connect_subgraph(self, condition_is_true_or_false):
        ...
        if condition_is_true_or_false:
            for uid in self.subgraph_final_ids:
                mygraph.graph.add_new_unstarted_connection(
                    (rename_dict[uid], nextgraph.uid)
                )
...
class WorkflowBase(XMLYMLInstantiationBase):
    _fields = [
        ...
        (
            ...
            str,
            "",
            "Path to the storage directory assigned by the workflow client.",
            "a",
        ),
        (
            "name",
            str,
            "Workflow",
            "Name of this workflow. Something like Hans or Fritz.",
            "a",
        ),
        (
            "submit_name",
            str,
            "${SUBMIT_NAME}",
            "The name this workflow was submitted as. This has to be unique on the cluster (per user). The workflow will be rejected if its not.",
            "a",
        ),
        ...
        (
            ...
            str,
            "unset",
            "Only present for legacy purposes. Do not use.",
            "a",
        ),
    ]
        ...
        for node in self.graph.report_order_generator():
            try:
                ...
                for element in body_html.iter():
                    ...
                    body_html.iter()
                single_element = etree.tounicode(
...
class Workflow(WorkflowBase):
            ...
            if running.completed_or_aborted():
                try:
                    ...
                    if running.uid not in self._input_hashes:
                        self._logger.warning(
                            f"[REPO] Result of {running} could not be stored in result database because its hash was not \
                    ...
                    with (runtime_dir / "security_error.log").open("w") as outfile:
                        if isinstance(e, jsonschema.exceptions.ValidationError):
                            outfile.write(
                                f"Encountered security error. Error was: {e.message}."
                            )
                        else:
                            outfile.write(
                                f"Encountered security error. Error was: {e}."
                            )
                    ...
                    self._logger.error(
                        "Aborting workflow %s due to error in Job." % self.submit_name
                    )
                    self.abort()
                    return True
                except Exception:
                    self._logger.exception(
                        "Exception during postjob care. Aborting workflow."
                    )
                    self.abort()
                    return True
            ...
            if isinstance(tostart, WorkflowExecModule):
                ...
                try:
                    if not self._prepare_job(tostart):
                        self._logger.error(
                            "Error during job preparation. Aborting workflow %s"
                            % self.submit_name
                        )
                        self.abort()
                        return True
                        ...
                        if input_hash in self._input_hashes:
                            self._logger.warning(
                                f"[REPO] A WFEM with the same hash as {tostart.outputpath} ({self._input_hashes[input_hash].outputpath}) has already been started in this workflow! \
                            ...
                            if tostart.resources.reuse_results:
                                (
                                    found_result,
                                    original_path,
                                ) = self._result_repo.load_results(input_hash, tostart)
                                if found_result:
                                ...
                                self._logger.info(
                                    f"[REPO] Any existing results will not be reused for {tostart.outputpath} since the feature is disabled by the resource configuration."
                                )
                        if not self._check_if_job_is_local(tostart):
                            external_cluster_manager = (
                                self._get_clustermanager_from_job(tostart)
                            )
                        else:
                        ...
                        self._logger.info(
                            "Started job >%s< in directory <%s> ."
                            % (rdjob, tostart.runtime_directory)
                        )
                except Exception as e:
                    print(e)
                    traceback.print_exc()
                    ...
                    tostart.set_failed()
                    self._logger.exception(
                        "Uncaught exception during job preparation. Aborting workflow %s"
                        % self.submit_name
                    )
                    self.abort()
                    return True
                ...
                if secure_mode:
                    raise WorkflowAbort(
                        "Secure Mode WFs must not contain VariableElements."
                    )
                result = tostart.evaluate_equation(
                ...
                isinstance(tostart, ForEachGraph)
                or isinstance(tostart, IfGraph)
                or isinstance(tostart, WhileGraph)
            ):
                if secure_mode:
                    raise WorkflowAbort(
                        "Secure Mode WFs must not contain dynamic elements such as If, ForEach or While."
                    )
                (
                    new_connections,
                    new_activity_elementlists,
                    new_graphs,
                ) = tostart.resolve_connect(
                    base_storage=self.storage,
                    input_variables=self._input_variables,
                    output_variables=self._output_variables,
                )
    ...
    def _prepare_job(self, wfem: WorkflowExecModule):
        ...
        secure_mode = SecureModeGlobal.get_secure_mode()
        jobdirectory = (
            self.storage + "/exec_directories/" + self._get_job_directory(wfem)
        )
        counter = 0
        while os.path.isdir(jobdirectory):
            ...
            if counter == 20:
                raise WorkflowAbort(
                    "Job directory could not be generated even after 20 attempts."
                )
        ...
        try:
            ...
            self._logger.info(
                "Did not find wano_configuration.json. This is ok, if this was an old client using the V1 workflow format."
            )
        ...
        for myinput in wfem.inputs:
            ...
            for myfile in allfiles:
                if not path.isfile(myfile):
                    self._logger.error(
                        "Could not find file %s (expected at %s) for wanoid %s on disk. Canceling workflow. Target was: %s"
                        % (source, absfile, wfem.uid, tofile)
                    )
                    return False
        ...
        for myinput in wfem.inputs:
            ...
            for absfilenum, absfile in enumerate(allfiles):
                if not path.isfile(absfile):
                    self._logger.error(
                        "Could not find file %s (expected at %s) on disk. Canceling workflow. Target was: %s"
                        % (source, absfile, tofile)
                    )
                    return False
                ...
                try:
                    if (
                        absfile.endswith("output_dict.yml")
                        or absfile.endswith("output_config.ini")
                        or absfile.endswith("report_template.body")
                    ):
                        raise JustCopyException()
                    with open(absfile, "r") as infile:
                        absfile_content = infile.read()
                    rendered_content = Template(absfile_content).render(
                        wano=rendered_wano,
                        input_variables=self._input_variables,
                        output_variables=self._output_variables,
                    )
                ...
                except JustCopyException:
                    shutil.copyfile(absfile, tofile)
                except Exception as e:
                    self._logger.warning(
                        "Unable to render input file %s. Copying instead. Exception was: %s"
                        % (absfile, e)
                    )
                    shutil.copyfile(absfile, tofile)
                if do_aiida:
                    if absfile in self._filepath_to_aiida_uuid:
                        ...
                        afile = SinglefileData(
                            actual_tofile, filename=actual_tofile_rel
                        )
        ...
        if do_aiida:
            ...
            aiida_files.append(
                SinglefileData(
                    "%s/rendered_wano.yml" % jobdirectory, filename="rendered_wano.yml"
                )
            )
            ...
            for logical_path, fileobj in aiida_files_by_relpath.items():
                aiida_rw["filename_locations"]["a%s" % fileobj.uuid] = orm.Str(
                    logical_path
                )
    ...
    def _postjob_care(self, wfem: WorkflowExecModule):
        ...
        if not wfem.check_if_job_is_local():
            ...
            self._logger.info(
                f"Retrieving runtime directory of non-local job from {ext_runtime_dir} to {runtime_dir}."
            )
            ...
            for myoutput in outputs:
                ...
                try:
                    ...
                    if myoutput not in ["retrieved", "remote_folder"]:
                        self._logger.exception(
                            "Expected Exception here trying to stage out %s" % myoutput
                        )
                        self._logger.error(
                            "keys were: %s"
                            % (
                                ",".join(
                                    [str(key) for key in aiida_to_simstack_pathmap]
                                )
                            )
                        )
        ...
        for myoutput in wfem.outputs:
            ...
            if secure_mode:
                if output != "output_dict.yml":
                    ...
                    with (Path(jobdirectory) / "security_error.log").open(
                        "w"
                    ) as outfile:
                        outfile.write(message)
                    raise SecurityError(message)
            ...
            if "*" in absfile:
                ...
                if len(allfiles) == 0:
                    mystdout = (
                        "Tried staging out multiple files matching %s, but did not find a single file."
                        % absfile
                    )
                ...
                if not path.isfile(absfile):
                    mystdout = (
                        "Could not find outputfile %s on disk (requested at %s). Canceling workflow."
                        % (output, absfile)
                    )
                    self._logger.error(mystdout)
                    raise WorkflowAbort(mystdout)
        ...
        for myoutput in wfem.outputs:
            ...
            for tocopy in allfiles:
                if not path.isfile(tocopy):
                    mystdout = (
                        "Could not find outputfile %s on disk. Canceling workflow."
                        % output
                    )
                    self._logger.error(mystdout)
                    raise WorkflowAbort(mystdout)
                ...
                if tocopy in staging_filename_to_aiida_obj:
                    self._filepath_to_aiida_uuid[
                        tofile
                    ] = staging_filename_to_aiida_obj[tocopy].uuid
    ...
    def get_jobs(self):
        ...
        alljobobjs = [
            self.elements.get_element_by_uid(job) for job in alljobs if job != "0"
        ]
        return alljobobjs
    ...
    def get_running_finished_job_list_formatted(self):
        files = []
        success, failed, running = self.graph.get_success_failed_running_jobs()
        for joblist, status in zip(
            [running, failed, success],
            [JobStatus.RUNNING, JobStatus.FAILED, JobStatus.SUCCESSFUL],
        ):
            for job in joblist:
                if job == "0":
                ...
                self._logger.debug(
                    "Looking for commonpath between %s and %s"
                    % (jobobj.runtime_directory, self.storage)
                )
                ...
                if jobobj.runtime_directory.startswith("/"):
                    commonpathlen = len(
                        path.commonpath([jobobj.runtime_directory, self.storage])
                    )
                else:
                    commonpathlen = 0
                ...
                jobdict = {
                    ...
                    "path": jobdir,
                    "status": status,
                    "original_result_directory": jobobj.original_result_directory
                    if jobobj.original_result_directory != ""
                    else None,
                }
